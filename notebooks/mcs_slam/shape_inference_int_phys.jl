...
scene_data, status = mcs.load_scene_json_file(
    "../../../../GenPRAM_workspace/GenPRAM/assets/eval_4_all/quebec_0001_06.json")
step_metadata_list = []
...
# +
timesteps = collect(92:93);
t = timesteps[1]
# t = 90
V.reset_visualizer()

V.viz(obs_clouds[t] ./ 10.0; color=I.colorant"black", channel_name=:h2)
# V.viz(obs_clouds[t+1] ./ 10.0; color=I.colorant"red", channel_name=:h1)
...
score_clouds(obs_cloud, gen_cloud) = Gen.logpdf(T.uniform_mixture_from_template, obs_cloud, gen_cloud, 0.0001, 2*voxelize_resolution, (-100.0, 100.0, -100.0, 100.0,-100.0,100.0))
@show score_clouds(obs_clouds[t+1], obs_clouds[t])
@show score_clouds(obs_clouds[t+1], obs_clouds[t+1])

# +
cam = GL.scale_down_camera(camera, 2)
...
c = GL.depth_image_to_point_cloud(d, cam)
V.reset_visualizer();V.viz(c)

# +
i = 3
V.reset_visualizer();V.viz(centered_entities[i])
mins,maxs = T.min_max(centered_entities[i])
mins = mins .- 0.1
maxs = maxs .+ 0.1
RESOLUTION = voxelize_resolution
...
occupied, occluded, free = T.get_occ_ocl_free(T.move_points_to_frame_b(ALL_VOXELS,start_poses[i]),
    camera, depth_images[t], voxelize_resolution);
sum(occupied) ./ length(occupied)
V.reset_visualizer()
V.viz(ALL_VOXELS[:, occluded] ./ 2.0; color=I.colorant"red", channel_name=:h1)
V.viz(ALL_VOXELS[:, occupied] ./ 2.0; color=I.colorant"black", channel_name=:h2)
